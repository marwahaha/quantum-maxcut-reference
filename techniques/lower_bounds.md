...
Note that this state is not great for QMC or XY, as it obtains energy $$0$$ and $$1/2$$ respectively on each unweighted edge, theryby doing worse or equivalent to the [Maximally Mixed State](#maximally-mixed-state). The intution is that QMC and XY encode projectors onto *asymmetric* states, and fully symmetric states like the Zero State lie in an orthogonal subspace to this image.
...
Cut States are useful because we have [upper bounds based on cuts]({{site.baseurl}}/techniques/upper_bounds.html#cut-bound). This allows us to directly relate the lower and upper bounds in terms of cuts, allowing us to derive approximation ratios.
