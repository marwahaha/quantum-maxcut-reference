...
3. **Randomized rounding:** for each vertex $$i$$, sample a pure state $$|\Psi\rangle$$ from the distribution defined by $$\rho_i$$ (or deterministically pick the closest pure state if derandomization is desired).  
