...
<div style="position:absolute;top:10px;right:12px;display:flex;flex-direction:column;gap:6px">
  <div style="background:var(--color-background-primary);border:0.5px solid var(--color-border-tertiary);border-radius:8px;padding:8px 10px">
    ...
    <div style="display:flex;gap:4px">
      <button id="bax" onclick="setAxis('alpha')" style="font-size:11px;padding:3px 8px;border-radius:5px;border:0.5px solid var(--color-border-secondary);cursor:pointer;background:var(--color-background-secondary);color:var(--color-text-primary);font-family:var(--font-sans)">α</button>
      <button id="bbx" onclick="setAxis('beta')" style="font-size:11px;padding:3px 8px;border-radius:5px;border:0.5px solid var(--color-border-secondary);cursor:pointer;background:var(--color-background-secondary);color:var(--color-text-primary);font-family:var(--font-sans)">β</button>
      <button id="bcx" onclick="setAxis('gamma')" style="font-size:11px;padding:3px 8px;border-radius:5px;border:0.5px solid var(--color-border-secondary);cursor:pointer;background:var(--color-background-secondary);color:var(--color-text-primary);font-family:var(--font-sans)">γ</button>
    </div>
...
for(let i=0;i<N;i++){
  for(let j=0;j<N;j++){
    for(let k=0;k<N;k++){
      ...
      const b = range[0]+j*step;
      const g = range[0]+k*step;
      if(a<b-0.001||b<g-0.001) continue;
      const cls = classify(a,b,g);
      positions_arr.push(a,b,g);
      const c = classColor(cls);
      colors.push(c.r,c.g,c.b);
      points.push({a,b,g,cls});
    }
  }
}
...
const mat = new THREE.PointsMaterial({
  size:0.07, vertexColors:true, sizeAttenuation:true, transparent:true, opacity:0.82
});
...
makeLabel('α', [1.45,0,0]);
makeLabel('β', [0,1.45,0]);
makeLabel('γ', [0,0,1.45]);
...
function setAxis(ax){
  sliceAxis = ax;
  document.getElementById('bax').style.fontWeight = ax==='alpha' ?'600':'400';
  document.getElementById('bbx').style.fontWeight = ax==='beta'  ?'600':'400';
  document.getElementById('bcx').style.fontWeight = ax==='gamma' ?'600':'400';
...
function buildSlice(){
  ...
  const sp=[], sc=[];
  points.forEach(pt => {
    const v = sliceAxis==='alpha' ? pt.a : sliceAxis==='beta' ? pt.b : pt.g;
    if(Math.abs(v-sliceVal)>eps) return;
    sp.push(pt.a, pt.b, pt.g);
  ...
  sliceMesh = new THREE.Points(sg, new THREE.PointsMaterial({
    size:0.12, vertexColors:true, sizeAttenuation:true, transparent:true, opacity:1.0
  }));
  scene.add(sliceMesh);
  cloud.visible = false;
}
...
function updateSize(v){
  mat.size = parseFloat(v)*0.023;
  if(sliceMesh) sliceMesh.material.size = parseFloat(v)*0.04;
}

let drag=false, lastX=0, lastY=0;
let theta=Math.PI*0.4, phi=0.42, radius=4.2, panX=0, panY=0;
...
function resetCam(){ theta=Math.PI*0.4; phi=0.42; radius=4.2; panX=0; panY=0; }
...
el.addEventListener('mousemove', e=>{
  ...
  if(hits.length>0){
    ...
    tooltip.innerHTML='α='+pt.a.toFixed(2)+', β='+pt.b.toFixed(2)+', γ='+pt.g.toFixed(2)+'<br><b>'+label+'</b>';
